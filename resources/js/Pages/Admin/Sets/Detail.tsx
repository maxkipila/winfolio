...
type Props = {
    set: SetLego
    otherSets: Array<SetLego>;
}

const Detail = (props: Props) => {

    const { name, img_url, year, num_parts, forecast, release_date, availability, packaging, pieces, minifigs_count, retail_price, market_price } = props.set
    const { otherSets } = props
    ...
    return (
        <AdminLayout rightChild={true} title="Detail Setu | Winfolio">
            ...
            <div className="p-[16px] ">
                <Breadcrumbs
                    previous={{ name: 'Seznam setů', href: route('admin.products.index.set') }}
                    current={`${name} `}
                />
                ...
                <div className='flex  items-center'>
                    ...
                    <div className='border justify-center mx-a ml-[150px] items-start p-24px flex-col border-gray-300  flex gap-[8px] border-b-0 border-r-0 rounded-b-0 rounded-r-0 rounded-md'>
                        <div className="font-bold text-black text-xl">Set Details</div>
                        <div className='flex gap-24px justify-between'>
                            <span className="text-base">Set number</span>
                            <span className="text-base ">{props.set.product_num}</span>
                ...
                <div className="   ">
                    ...
                    <div className="flex p-[16px] bg-[#F5F5F5] border-black border-2">
                        ...
                        <div className=' ml-24px flex w-full flex-col gap-[8px]'>
                            ...
                            <div className='text-[#4D4D4D] font-bold text-sm'>{props.set.theme.name}</div>
                            <div className='w-full border-[#D0D4DB] border-b'></div>
                            <div className='text-[#4D4D4D] font-medium text-sm'>{year}</div>
                    ...
                    <div className="flex-1 p-[16px] border-2 border-black border-t-0  grid grid-cols-2 gap-y-[8px] text-[14px]">
                        ...
                        <div className='text-black  font-bold  text-sm'>Set pricing</div>
                ...
                <div className=" mt-[39px] border-[2px] border-black borde-2 p-[16px] space-y-[8px]">
                                ...
                                return {
                                    ...
                                    element: (
                                        <SearchCard
                                            name={r.name}
                                            image={r.img_url}
                                            href={route('admin.products.show.set', { product: r.id })}
                                        />
                                    ),
                                    value: r.id,
                                    model: r,
                    ...
                    <div className=" w-full py-[12px] flex flex-wrap gap-[16px]">
                        {otherSets && otherSets.map((item) => (
                            <Link
                                key={item.id}
                                href={route('admin.products.show.set', { product: item.id })}
