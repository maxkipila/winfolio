...
class ScrapeBrickEconomy implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            $pages = $this->puppeteerRequest($urls);
            Log::info('finished the scraping', ['pages' => count($pages ?? [])]);
        } catch (\Exception $e) {
            $this->fail($e);
        }

        foreach ($pages ?? [] as $product_id => $html) {
            $this->parsePrices($html, $product_id);
            $this->parseImages($html, $product_id);
        }
    }
